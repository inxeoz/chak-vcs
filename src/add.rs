...
pub fn local_ignore_file_to_set(
    ignore_file_path: &Path,
    working_root_path: &Path,
) -> HashSet<PathBuf> {
    ...
    if let Ok(contents) = fs::read_to_string(ignore_file_path) {
        for line in contents.lines() {
            ...
            if !trimmed.is_empty() && !trimmed.starts_with('#') {
                ...
                let combined_ignore_from_root =
                    resolve_path(&ignore_file_parent.join(trimmed), working_root_path);
...
pub fn global_ignore_file_to_set(
    global_ignore_file_path: &Path,
    working_root_path: &Path,
) -> HashSet<PathBuf> {
    ...
    if let Ok(contents) = fs::read_to_string(global_ignore_file_path) {
        for line in contents.lines() {
            ...
            if !trimmed.is_empty() && !trimmed.starts_with('#') {
                ...
                // }
                let mut ignore_path =
                    resolve_path(&PathBuf::new().join(trimmed), working_root_path)
                        .canonicalize()
                        .unwrap();
...
pub fn parse_ignore_file(
    ...
    let local_ignore = local_ignore_file_to_set(ignore_file_path, working_root_path);
    let mut global_ignore = global_ignore_file_to_set(global_ignore_path, working_root_path);
...
pub fn snapshot_directory(
    ...
    for entry in fs::read_dir(dir_path)? {
        ...
        if path.is_dir() {
            let sub_snapshot = snapshot_directory(&path, blob_dir)?;
            snapshot.extend(sub_snapshot);
        } else {
            let hash = create_blob(&path, blob_dir)?;
...
pub fn filter_with_ignore(
    snapshot: HashMap<PathBuf, String>,
    ignore_rules: &HashSet<PathBuf>,
